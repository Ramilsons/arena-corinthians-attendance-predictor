...
* **Chronological Split (*Time-Based Split*):** Since we used Target Encoding, we structured the data split strictly chronologically (by date). This was fundamental to **prevent data leakage**, ensuring that the target mean calculated for a team's encoding considers only past matches, without "peeking into the future."
